...
// Build a gauge chart
function build_gauge_chart(sample) 
{
  d3.json(url).then((data) => 
  {
    let metadata = data.metadata;
...
function run() 
{
  ...
  d3.json(url).then((data) => 
  {
    ...
    build_meta_data(first_sample);
    build_gauge_chart(first_sample);
  });
};
...
function optionChanged(sample_fetch) 
{
  ...
  build_charts(sample_fetch);
  build_meta_data(sample_fetch);
  build_gauge_chart(sample_fetch);
};
